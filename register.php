...
if (isset($_POST['registerBtn'])) {
    ...
    if (!empty($username) && !empty($pass) && !empty($repass)) {
        ...
        if (empty($error_msg)) {
            $sql = "INSERT INTO user (username, password, role, is_locked) 
                VALUES ('$username', '$pass', 'customer', 0)";
    ...
    echo "<script>window.location = 'index.php?page=Dangky&error=" . urlencode($error_msg) . "';</script>";
}
?>
...
<div id="registerForm">
    ...
    <form action="" method="post">
        ...
        <button name="registerBtn" type="submit">Đăng ký</button>
    </form>
</div>
